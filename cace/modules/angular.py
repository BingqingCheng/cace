...
#added
    def forward(self, vectors: torch.Tensor) -> torch.Tensor:
        ...
        for l in range(1, self.l_max + 1):
            for lxlylz_combination in self.lxlylz_dict[l]:
                # parts = list(map(int, lxlylz_combination.split('_')))  # string to list
                # max_val = max(parts)
                # i = parts.index(max_val)
                # parts[i] -= 1
                # prev_key = "_".join(map(str, parts))  # key in string format
                ...
                for c in lxlylz_combination:
                    if c == '_':
                        ...
                        current_num += c
                parts.append(int(current_num))  # 마지막 숫자 추가
...
#         return [computed_values["_".join(map(str, comb))] for comb in self.lxlylz_list]
# #added
    def _convert_computed_values_to_list(self, computed_values: Dict[str, torch.Tensor]) -> List[torch.Tensor]:
