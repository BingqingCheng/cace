...
class MetalWallQEQ(nn.Module):
    def __init__(self,
                 ...
                 external_potential = None, # external potential applied to each metal atom, in array form
                 feature_key: str = 'q',
        ...
        for i in unique_batches:
            ...
            if metal_index.sum() == 0:
                ...
                need_recompute = (
                    ...
                    or not torch.allclose(self.r, r, atol=1e-2)
                ...
                if self.external_potential is not None:
                    B_ext = torch.tensor(self.external_potential, device=q_now.device, dtype=q_now.dtype) * self.external_field_norm_factor
                    assert r.shape[0] == B_ext.shape[0], 'external potential dimension error'
