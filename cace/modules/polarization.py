...
__all__ = ['Polarization', 'Dephase', 'FixedCharge', 'Quadrupole']
...
class Quadrupole(nn.Module):
    ...
    def forward(self, data: Dict[str, torch.Tensor], training=True, output_index=None) -> torch.Tensor:
        ...
        for i in unique_batches:
            ...
            rij = r_now[:,:,None] * r_now[:,None,:]
            quad = (q_now.reshape(-1,1,1) * rij).sum(dim=0)
            eye = torch.eye(3).to(rij.device)
            quad = quad - 1/3*quad.trace()*eye
            results.append(quad * self.normalization_factor)
        data[self.output_key] = torch.stack(results, dim=0)
        return data
